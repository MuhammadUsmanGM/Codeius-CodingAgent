...
function App() {
  const [output, setOutput] = useState([
    { id: 1, text: 'Welcome to Codeius Terminal!', type: 'system' },
    { id: 2, text: 'Type a command or ask a question.', type: 'system' }
  ]);
  ...
  const fetchModels = async () => {
    try {
      ...
      // Add a message to the terminal output about the error
      setOutput(prev => [...prev, { 
        id: Date.now(), 
        text: `⚠️ Backend server not available. Please make sure the Codeius backend is running. Error: ${error.message}`, 
        type: 'error' 
      }]);
    }
  };
  ...
  const processCodeiusCommand = async (cmd) => {
    ...
    if (cmd === '/help') {
      const helpText = [
        ...
        '  /switch [key]  - Switches to a specific model',
        '  /mcp           - Lists MCP tools',
        '  /dashboard     - Shows dashboard',
        '  /themes        - Shows available themes',
        '  /clear         - Clears the terminal',
        '  /test          - Runs a test command',
        '  /exit          - Exits the application',
        '',
        'Or type any question to ask the AI!'
      ].join('\n');
      ...
      setOutput([...newOutput, { id: Date.now() + 1, text: 'Dashboard not implemented yet', type: 'output' }]);
    } else if (cmd === '/themes') {
    ...
    } else if (cmd === '/clear' || cmd === '/cls' || cmd === '/clear_screen') {
      setOutput([]);
    } else if (cmd === '/exit' || cmd === '/quit') {
      setOutput([...newOutput, { id: Date.now() + 1, text: 'Exiting Codeius...', type: 'system' }]);
    } else {
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'ArrowUp') {
      ...
      const commands = ['/help', '/models', '/switch', '/mcp', '/dashboard', '/themes', '/clear', '/test', '/exit'];
      ...
      <div className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-footer">
          <div className="current-model">
            <span>Current: {availableModels[currentModel]?.name || 'Loading...'}</span>
          </div>
          <select 
            value={currentModel} 
